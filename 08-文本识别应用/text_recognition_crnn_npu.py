...
parser.add_argument('--model', '-m', type=str, default='text_recognition_CRNN_EN_2022oct_int8.onnx', help='Path to the model.')
...
def visualize(image, boxes, texts, color=(0, 255, 0), isClosed=True, thickness=2):
    ...
    for box, text in zip(boxes[0], texts):
        cv.putText(output, text, (box[1].astype(np.int32)), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    return output

if __name__ == '__main__':
    # 初始化CRNN
    recognizer = CRNN(modelPath=args.model, charsetPath=args.charset, 
            backendId=cv.dnn.DNN_BACKEND_TIMVX, targetId=cv.dnn.DNN_TARGET_NPU)
    # 初始化DB
    detector = DB(modelPath='text_detection_DB_IC15_resnet18_2021sep.onnx',
    ...
    if args.input is not None:
        ...
        image = cv.resize(image, [original_w, original_h])
        ...
        cv.namedWindow('{} Demo'.format(recognizer.name), cv.WINDOW_NORMAL)
        tm = cv.TickMeter()
        while cv.waitKey(1) < 0:
            ...
            frame = cv.resize(frame, [original_w, original_h])
            # 显示图像
